...
import "./print.css";
import { getRecipeByItemId, useGetRecipe } from "@/services/UseRecipe";
...
export default function Outpost() {
  ...
  const onChangeRecipe = useCallback(
    async (index: number, itemAndCount: ItemAndCount) => {
      const itemInfo = producedOnOutpost[index];

      if (itemAndCount.item.id !== itemInfo?.recipe?.produced?.id) {
        const recipe = await getRecipeByItemId(itemAndCount.item.id);
        setProducedOnOutpost((oldProducedOnOutpost) => {
          ...
          newProducedOnOutpost[index] = {
            ...
            totalPerMinute:
              (recipe?.producingPerMinute || 0) * itemAndCount.count,
          } as ItemDeepInfo;
    ...
    producedOnOutpost
      ...
      .forEach((itemInfo) => {
        ...
        consumptionFromOneRecipe.forEach((itemAndCount) => {
          ...
          if (existedItem) {
            existedItem.count += itemAndCount.count * itemInfo.factoryCount;
          } else {
      ...
      <div className="flex gap-4">
        <div className="flex-1">
          ...
          {producedOnOutpost.map((itemInfo, index) => (
            <div
              className="one-factory"
              key={itemInfo?.recipe?.produced.id ?? 1}
            >
              <RecipeItemSelector
              ...
              {isProduceTargetSelected && (
                <h3 className="text-lg mb-4 production-info">
        ...
        {isProduceTargetSelected && (
          <div className="flex-1">
            <h3 className="text-lg mb-2 result">Закидываем на эту площадку</h3>
            <div>
              {totalConsumption.map(({ item, count }) => (
                <div key={item.id} className="one-factory">
                  <RecipeItemSelector item={item} count={count} readonly />
                </div>
              ))}
            </div>

            <h3 className="text-lg mb-2 result">Остаток</h3>
            <div>
